...
void ControlNode::popNextCommand(const tum_ardrone::filter_stateConstPtr statePtr)
{
	...
	while(currentKI == NULL && commandQueue.size() > 0)
	{
        ...
        //added part
        //gotogps
        else if(sscanf(command.c_str(),"gotogps %f %f %f",&parameters[0], &parameters[1], &parameters[2]) == 3)
        {
            SetGPSTarget_srv_srvs.request.target.position.latitude = parameters[0];
            SetGPSTarget_srv_srvs.request.target.position.longitude = parameters[1];
            SetGPSTarget_srv_srvs.request.target.position.altitude = parameters[2];
