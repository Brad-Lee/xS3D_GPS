...
EstimationNode::EstimationNode()
{
    ...
    //added part
    navdata_gps_pub    = nh_.advertise<ardrone_autonomy::navdata_gps>("ardrone/navdata_gps", 50);
    navdata__gps_sub = nh_.subscribe("ardrone/navdata_gps", 10, &EstimationNode::navdata_gpsCb, this);
...
//added part
void EstimationNode::navdata_gpsCb(const ardrone_autonomy::navdata_gpsConstPtr navdata_gpsPtr)
{
    lastnavdata_gpsReceived = *navdata_gpsPtr;
    if(ros::Time::now() - lastnavdata_gpsReceived.header.stamp > ros::Duration(30.0))
        lastnavdata_gpsReceived.header.stamp = ros::Time::now();
}
...
void EstimationNode::Loop()
{
	  ...
	  while (nh_.ok())
	  {
          ...
          //added part
          ardrone_autonomy::navdata_gps t;
          t.latitude = lastnavdata_gpsReceived.latitude;
          t.longitude = lastnavdata_gpsReceived.longitude;

		  // publish!
		  dronepose_pub.publish(s);
          //added part
          navdata_gps_pub.publish(t);
...
void EstimationNode::reSendInfo()
{
	...
	*/
    ...
    snprintf(buf,1000,"u s PTAM: %s\n%s\nScale: %.3f (%d in, %d out), acc: %.2f\nScaleFixpoint: %s\nDrone Status: %s (%d Battery)\n Latitude: %f \n Longitude: %f",
            ...
            status.c_str(), (int)lastNavdataReceived.batteryPercent, lastnavdata_gpsReceived.latitude, lastnavdata_gpsReceived.longitude);

    //added part
    ofstream myfile;
    myfile.open("/home/xs3d/battery.txt");
    myfile << (int)lastNavdataReceived.batteryPercent;
    myfile.close();

    //added part #include <iomanip>
    ofstream myfile_la;
    myfile_la.open("/home/xs3d/latitude.txt");
    myfile_la << fixed << lastnavdata_gpsReceived.latitude;
    myfile_la.close();

    //added part
    ofstream myfile_lo;
    myfile_lo.open("/home/xs3d/longitude.txt");
    myfile_lo << fixed << lastnavdata_gpsReceived.longitude;
    myfile_lo.close();
