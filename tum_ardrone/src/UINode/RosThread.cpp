...
void RosThread::run()
{
    ...
    //added part
    SetGPSTarget_srv     = nh_.serviceClient<ardrone_autonomy::SetGPSTarget>(nh_.resolveName("ardrone/setgpstarget"), 1);
    SetAutoflight_srv     = nh_.serviceClient<ardrone_autonomy::RecordEnable>(nh_.resolveName("ardrone/setautoflight"), 1);
...
//added part
void RosThread::sendSetGPSTarget()
{
    pthread_mutex_lock(&send_CS);

//    SetGPSTarget_srv_srvs.request.target.position.latitude = 36.366931;
//    SetGPSTarget_srv_srvs.request.target.position.longitude = 127.363633;
//    SetGPSTarget_srv_srvs.request.target.position.altitude = 1.5;

    SetGPSTarget_srv.call(SetGPSTarget_srv_srvs);

    pthread_mutex_unlock(&send_CS);
}
void RosThread::sendSetAutoflight()
{
    pthread_mutex_lock(&send_CS);
    SetAutoflight_srv_srvs.request.enable = 1;
    SetAutoflight_srv.call(SetAutoflight_srv_srvs);
    pthread_mutex_unlock(&send_CS);
}
